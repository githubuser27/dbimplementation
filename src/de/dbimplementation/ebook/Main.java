 ...
 * 
 */
package de.dbimplementation.ebook;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		HGHandle handleFamA = graph.add(new HGPlainLink(handleAV, handleAM, handleAT1, handleAT2, handleAS1));
		HGHandle handleFamB = graph.add(new HGPlainLink(handleBV, handleBM, handleBT1, handleBT2));
		
		// Link friendships with HGValueLink
		HGHandle handleFs1 = graph.add(new HGValueLink(29, handleAV, handleBV));
		HGHandle handleFs2 = graph.add(new HGValueLink(11, handleAM, handleBM));
		HGHandle handleFs3 = graph.add(new HGValueLink(10, handleAT2, handleBT2));
		HGHandle handleFs4 = graph.add(new HGValueLink(4, handleAS1, handleBT1));
		
		Person persAV = graph.get(handleAV);
		HGPlainLink plFamA = graph.get(handleFamA);
		HGValueLink vlFs1 = graph.get(handleFs1);
		
		for (int a=0; a<plFamA.getArity(); a++) {
			HGHandle targetOfLink = plFamA.getTargetAt(a);
		}
		
		vlFs1.setValue((int)10);
		int value = (int) vlFs1.getValue();
		
		HGQueryCondition cond = new And(
				new AtomTypeCondition(Person.class), 
				new AtomPartCondition(new String[]{"name"}, new String("AV"), ComparisonOperator.EQ)
				);
		HGSearchResult<HGHandle> result = graph.find(cond);
		while(result.hasNext()) {
			HGHandle h = result.next();
		}
		result.close();
